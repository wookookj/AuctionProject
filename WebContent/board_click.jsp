...
%>
	...
	<script>
		...
		function board_delete(form) {
			location.href= "board_delete_db.jsp?num=" + form.num.value + "&id=" + form.id.value;
		}
		function board_update(form) {
			location.href= "board_update.jsp?num=" + form.num.value;
		}	
		...
		<form>
			<jsp:useBean id ="board" class ="Action_projcet.Member_Board_DAO" />
				<% int num = Integer.parseInt(request.getParameter("num")); 
					// 게시물 클릭시 게시물 내용 호출하는 메소드
					Board_DTO dto = board.board_db_select_num(num);
					// 게시물 클릭시 조회수 1씩 증가 메소드  *게시글 하나에  한번만 증가
					...
					if (session.getAttribute(num+":cookie") == null) {
				 	...
				 	if (!session.getAttribute(num+":cookie").equals(session.getAttribute(num+":cookie ex"))) {
				 		board.board_db_update_hits(num, dto.getHits());	
				 	}					
				%> 
			...
			<div>
				<table border = "0" cellspacing = "0">
					...
					<tr>
						<td>제목</td>
						<td><input type = "text" name = "title" value = "<%= dto.getTitle() %>" readonly="true"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name = "main" cols="30" rows ="25" readonly="true"><%= dto.getMain() %></textarea></td>
					</tr>
					<% if (dto.getId().equals("admin")) { %>		
					<%} else { %>
					<tr>
						<td>작성자</td>
						<td><input type = "text" name = "id" readonly="true" value = "<%= dto.getId() %>"></td>
					</tr>
					<% } %>
					<tr>
						<td>조회수</td>
						<td><input type = "text" name = "hits" readonly="true" value = "<%= dto.getHits() %>"></td>
					</tr>																																											
				</table>
			</div>
			<div> <br>
				<% if (dto.getId().equals("admin")) { %> 
					<input type="button" value = "목록" onclick = "location.href='community_adminboard.jsp'" class = "button">		
				<% } else { %>
					<input type="button" value = "목록" onclick = "location.href='community_freeboard.jsp'" class = "button">	
				<% } %>			
