    ...
    $(document).ready(function() {
		...
		function board_dat_update(form) {
			if(form.dat_update_content.value == "") {
				...
				location.href = "boarddatupdatePro.bo?dat_board_num=" + form.dat_update_num.value +"&board_num=" + form.num.value +"&board_content=" + encodeURIComponent(form.dat_update_content.value) + "&page=" + <%= nowPage %>	
			}
		}
				...
				%>	
					<% if(String.valueOf(dat_dto.get(i).getDat_num()).equals(dat_board_num) ) {%>
						...
						<div class = "board_dat_update">
								...
								<div><textarea name ="dat_update_content"><%= dat_dto.get(i).getDat_content() %></textarea></div>
								<div><input type = "button" value="취소" class="button" onclick="location.href = 'boardDetail.bo?board_num=' + <%= dto.getBOARD_NUM() %> + '&page=' + <%= nowPage%>"> <input type = "button" value="수정" class="button" onclick="board_dat_update(this.form)"></div>
						</div>
					<% } else {%>
					<% if(dat_dto.get(i).getDat_re_lev() > 0 ) {%> 	
						<div class = "board_dat_read_dat">						
							...
							<div>
								...
								<%if (dat_dto.get(i).getDat_re_lev()>1) {%>
